...
public class UserDBHelper extends SQLiteOpenHelper {
    ...
    public long insert(List<UserInfo> infoList) {
        ...
        for (int i = 0; i < infoList.size(); i++) {
            ...
            cv.put("age", info.age);
            cv.put("study", info.study);
            cv.put("game", info.game);
            cv.put("married", info.married);
            cv.put("update_time", info.update_time);
            cv.put("phone", info.phone);
            cv.put("password", info.password);
            cv.put("remnber", info.remnber);
            cv.put("remnber2", info.remnber2);
    ...
    public List<UserInfo> query(String condition) {
        String sql = String.format("select rowid,_id,name,age,study," +
                "game,married,update_time,phone,password,remnber,remnber2 " +
        ...
        while (cursor.moveToNext()) {
            ...
            info.study = cursor.getLong(4); // 取出长整型数
            info.game = cursor.getFloat(5); // 取出浮点数
            ...
            info.remnber = cursor.getString(10); // 取出字符串
            info.remnber2 = cursor.getString(11); // 取出字符串
            infoList.add(info);
        }
